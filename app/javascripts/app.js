...
const onBuyProduct = function (event) {
  ...
  Marketplace.deployed().then(function (contract) {
    contract.buy(productId, buyerContact, quantity, { value: amount * quantity, from: web3.eth.accounts[0], gas: 1000000 }).then(
      function (f) {
        $('#msg').show()
